...
public class ContaDAO {
    ...
    public List<ContaDTO> listar() throws Exception {
        ...
        try (Connection conexao = bd.conectar(); 
                PreparedStatement comando = conexao.prepareStatement(
                "SELECT id, numero, saldo, limite, cliente_id FROM conta"); 
                ...
                )
        {
            while (tabela.next()) {
                ...
                contaDTO.setClienteId(tabela.getInt("cliente_id"));
        ...
        try (Connection conexao = bd.conectar(); 
            ...
            try (ResultSet tabela = comando.executeQuery()) {
                while (tabela.next()) {
                    ...
                    contaDTO_.setClienteId(tabela.getInt("cliente_id"));
    ...
    public void alterar(ContaDTO contaDTO) throws Exception {
        try (Connection conexao = bd.conectar(); 
                PreparedStatement comando = conexao.prepareStatement(
                "UPDATE conta SET numero = ?, saldo = ?, limite = ?, cliente_id = ? WHERE id = ?")) {
            comando.setInt(1, contaDTO.getNumero());
            comando.setDouble(2, contaDTO.getSaldo());
            comando.setDouble(3, contaDTO.getLimite());
            comando.setDouble(4, contaDTO.getClienteId());
            comando.setInt(5, contaDTO.getId());
